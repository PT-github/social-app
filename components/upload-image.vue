          ...
          <view class="uni-uploader__files">
            <block v-for="(image,index) in imageList" :key="index">
              <view class="uni-uploader__file">
                ...
                <text class="iconfont icon-delete" @click="handleDelete(index)"></text>
              </view>
            </block>
    ...
    methods: {
      chooseImage: async function() {
        ...
        uni.chooseImage({
          ...
          count: this.imageList.length + this.count[this.countIndex] > 9 ? 9 - this.imageList.length : this.count[
            this.countIndex],
          success: (res) => {
            this.$emit('filechange', this.imageList.concat(res.tempFilePaths))
          },
          fail: (err) => {
            // #ifdef APP-PLUS
      ...
      handleDelete(index) {
        uni.showModal({
          ...
          content: '是否要删除该图片',
          success: res => {
            let array = this.imageList.slice()
            array.splice(index, 1)
            if (res.confirm) {
              this.$emit('filechange', array)
            }
          }
        })
      },
